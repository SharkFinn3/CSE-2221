...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        Map<String, Statement> oldMap = this.newContext();
        Map<String, Statement> newMap = this.newContext();
        this.swapContext(oldMap);

        while (oldMap.size() > 0) {
            printSpaces(out, Program.INDENT_SIZE);
            Map.Pair<String, Statement> p = oldMap.removeAny();
            out.println("INSTRUCTION " + p.key() + " IS");
            p.value().prettyPrint(out, 2 * Program.INDENT_SIZE);
            printSpaces(out, Program.INDENT_SIZE);
