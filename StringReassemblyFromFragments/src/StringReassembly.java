...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // creates a substring of str1 from its beginning to the beginning of the overlap
        String str1Substring = str1.substring(0, str1.length() - overlap);
        //concatenates the two strings, with the overlap removed from one of them
        String combination = str1Substring + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        temp.transferFrom(strSet); //creates empty Set and swaps with strSet
        while (temp.size() > 0) {
            String x = temp.removeAny();//removes any random string to compare
            if (x.contains(str)) { //if x contains str, cont increases
                cont = 2;
            }
            strSet.add(x); //x is placed back into strSet
        }
        if (cont == 1) { //if cont was incremented, this if statement gets skipped
            Set<String> temp2 = strSet.newInstance();
            temp2.transferFrom(strSet); //creates another empty set, and swaps again with strSet
            while (temp2.size() > 0) {
                String x = temp2.removeAny();//removes random string again to compare
                if (!str.contains(x)) { //this time if str doesn't contain x,
                    strSet.add(x); //x is placed into strSet
                }

            }
            strSet.add(str); //str is placed into strSet if cont never gets incremented
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> lines = new Set1L<>(); //creates new set
        while (!input.atEOS()) { //as long as line is in the stream, it gets placed into set
            addToSetAvoidingSubstrings(lines, input.nextLine());
            //calls addToSetAvoidingSubstrings to make sure no repeats
        }
        /*
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String newText = ""; //makes new empty string
        for (int i = 0; i <= text.length(); i++) {//iterates entire length of string
            newText = text.replace("~", "\n");//replaces all instances of "~" with a new line
        }
        out.print(newText); //outputs the new string with "~" gone and line separations
    }
